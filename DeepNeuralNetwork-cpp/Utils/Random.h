...
namespace internal {
	...
	inline void shuffle(int* arr, const int arr_size, RNG& rng) {
	...
	int create_shuffled_batches(
		...
		if (y.cols() != nobs) {
			throw std::invalid_argument("Input X & Y have different number of observations");
		}
		...
		// Compute batch size
		batch_size = (batch_size > nobs) ? nobs : batch_size;
